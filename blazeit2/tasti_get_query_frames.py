# get frames in jackson where object detector thinks there are 4+ cars
...
sel_frames_fname = os.path.join('./outputs/', query_name, 'tasti_frames.json')
...
print('load selected frames')
with open(sel_frames_fname, 'r') as f:
	sel_frames = json.load(f)
...
picked_frames = []
detector_count = 0
for video_id, frame_idx in sel_frames:
